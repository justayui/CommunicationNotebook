...
**受入基準**
- 検索窓への入力で、本文に対象文字列を含む伝達事項のみが非同期でリアルタイム抽出される(notesにタイトルカラムは存在しないため、検索対象は本文のみとする)。
...
お気に入りの一覧表示(「お気に入り」タブでの絞り込み表示)は、F5(絞込検索)の`GET /api/notes`側でまとめて対応する。本タスクでのバックエンド実装スコープは登録・解除APIのみとする。
</details>
...
- 認証・認可方式: 認証はSpring Securityによる自前のセッション方式管理とする。本アプリは社内限定・外部非公開の小規模ツールであるため、CSRF対策は本リリースでは無効化する(SPA+Cookieセッション構成における一般的なCSRFリスクは許容する判断とし、プロダクトオーナーと合意済み)。
...
9. セルフサインアップ時に、管理者が事前登録した職員IDの一覧(許可リスト)の中からのみ登録できるようにする。承認待ちを発生させる管理者作成方式とは異なり、即時利用開始できる手軽さは維持したまま、他者IDへのなりすまし登録や無関係な文字列での登録を防ぐ。
